...
export class MqttEventBusAdapter extends AbstractEventBusAdapter {

  static ADAPTER_NAME = 'mqtt';
  ...
  loadDependencies() {
    try {
      if (!MqttEventBusAdapter.Reader && !MqttEventBusAdapter.Writer) {
        require('mqtt');
        MqttEventBusAdapter.Reader = require('./MqttReader').MqttReader;
        MqttEventBusAdapter.Writer = require('./MqttWriter').MqttWriter;
      }
    } catch (err) {
